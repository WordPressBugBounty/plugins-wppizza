	...
	***************************************************************************************************************************************************/
	function do_update($installed_version, $force_update, $reset_admin_caps){
		...
		*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\*/
		...
		/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*
		*
		*	update template values (conditional on versions)
		*	@since 3.15
		*
		*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\/*\*/
		$this->update_template_values($blog_ids, $installed_version);
...
//		$option_parameters = array();
	...
	**********************************************/
	...
	/*********************************************
	*	v3.x update TEMPLATE OPTION values
	*	USE FOR UPDATES FROM VERSIONS 3.x -> xx
	*	@return void
	**********************************************/
	function update_template_values($blog_ids, $installed_version){

		/*===============================================================================
		#
		#	only updates from < 3.15
		#	(3.15 adds fixed surcharges option which we want to add to templates)
		#
		===============================================================================*/
		if(version_compare( $installed_version , '3.15', '<' )){

			$types = array('emails', 'print');
			...
			**********************************************/
			foreach($blog_ids as $blog_id => $table_prefix){
				...
				/***********************
					loop through emails and print
				***********************/
				foreach($types as $type){

				 	//option name
				 	$template_option_name = WPPIZZA_SLUG.'_templates_'.$type.'';

				 	//option values
				 	$vals = get_option(''.$template_option_name.'');
				 	...
				 	******************************/
				 	foreach($vals as $v => $val ){
				 		...
				 		//
				 		#splice 'surcharges' into sort->summary values for each template
				 		//
				 		$spliced_sort = array();
				 		foreach($vals[$v]['sort']['summary'] as $s => $enabled){
				 			//splice before taxes */
				 			if($s == 'taxes'){
				 				$spliced_sort['surcharges']  = 1;
				 			}
				 			//no duplicates (there should not be any anyway, but just in case)
				 			if(!in_array($s, array('surcharges', 'surcharge_0', 'surcharge_1', 'surcharge_percent', 'surcharge_fixed'))){
				 				$spliced_sort[$s] = $enabled;
				 			}

				 		}
				 		//replace old with new
				 		$vals[$v]['sort']['summary'] = $spliced_sort;

				 		//
				 		#splice 'surcharges' into section->summary->parameters values for each template
				 		//
				 		$spliced_sections = array();
				 		foreach($vals[$v]['sections']['summary']['parameters'] as $s => $enabled){

				 			//splice before taxes */
				 			if($s == 'taxes'){
				 				//section
				 				$spliced_sections['surcharges']['enabled']  = 1;
				 			}
				 			//no duplicates (there should not be any anyway, but just in case)
				 			if(!in_array($s, array('surcharges', 'surcharge_0', 'surcharge_1', 'surcharge_percent', 'surcharge_fixed'))){
				 				$spliced_sections[$s] = $enabled;
				 			}

				 		}
				 		//replace old with new
				 		$vals[$v]['sections']['summary']['parameters'] = $spliced_sections;
				 	}
				 	...
				 	******************************/
					if(is_multisite()){
						update_blog_option($blog_id, $template_option_name, $vals);
					}else{
						update_option($template_option_name, $vals, false);
					}
				}
			}
		}
