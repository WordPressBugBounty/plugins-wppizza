	...
	function options_validate($options, $input){
		...
		**/
		...
		/**
			allergens
		**/
		if(isset($_POST[''.WPPIZZA_SLUG.'_allergens'])){
			$this->wpml_register_allergens($options, true);
		}		
		/**
			foodtype
		**/
		if(isset($_POST[''.WPPIZZA_SLUG.'_foodtype'])){
			$this->wpml_register_foodtype($options, true);
		}	
...
********************************************************************************************************************************************************/
	...
	**/
	function wpml_strings($wppizza_options){
		...
		**/
		...
		/**
			allergens
		**/
		foreach($wppizza_options['allergens'] as $k=>$arr){
			$wppizza_options['allergens'][$k]['name'] = icl_translate(WPPIZZA_SLUG,'allergens_'. $k.'_name', $arr['name']);
			$wppizza_options['allergens'][$k]['sort'] = icl_translate(WPPIZZA_SLUG,'allergens_'. $k.'_sort', $arr['sort']);
		}
		...
		**/
		...
		/**
			foodtype
		**/
		foreach($wppizza_options['foodtype'] as $k=>$arr){
			$wppizza_options['foodtype'][$k]['name'] = icl_translate(WPPIZZA_SLUG,'foodtype_'. $k.'_name', $arr['name']);
			$wppizza_options['foodtype'][$k]['sort'] = icl_translate(WPPIZZA_SLUG,'foodtype_'. $k.'_sort', $arr['sort']);
		}
	...
	/**
		...
		wpml strings
		@since 3.6.2
		@sets/alters global wppizza wpml'd options
	**/
	function wpml_strings_wpml4(){
		...
		**/
		foreach($wppizza_options['allergens'] as $k=>$arr){
			$wppizza_options['allergens'][$k]['name'] = apply_filters('wpml_translate_single_string', $arr['name'], WPPIZZA_SLUG, 'allergens_'. $k.'_name', $wpml_session_lng);
			$wppizza_options['allergens'][$k]['sort'] = apply_filters('wpml_translate_single_string', $arr['sort'], WPPIZZA_SLUG, 'allergens_'. $k.'_sort', $wpml_session_lng);
		}
		...
		**/
		foreach($wppizza_options['additives'] as $k=>$arr){
			$wppizza_options['additives'][$k]['name'] = apply_filters('wpml_translate_single_string', $arr['name'], WPPIZZA_SLUG, 'additives_'. $k.'_name', $wpml_session_lng);
			$wppizza_options['additives'][$k]['sort'] = apply_filters('wpml_translate_single_string', $arr['sort'], WPPIZZA_SLUG, 'additives_'. $k.'_sort', $wpml_session_lng);
		}
		...
		**/
		foreach($wppizza_options['foodtype'] as $k=>$arr){
			$wppizza_options['foodtype'][$k]['name'] = apply_filters('wpml_translate_single_string', $arr['name'], WPPIZZA_SLUG, 'foodtype_'. $k.'_name', $wpml_session_lng);
			$wppizza_options['foodtype'][$k]['sort'] = apply_filters('wpml_translate_single_string', $arr['sort'], WPPIZZA_SLUG, 'foodtype_'. $k.'_sort', $wpml_session_lng);
		}
...
********************************************************************************************************************************************************/
	...
	*/
	function wpml_register_all_strings($options = null){
		...
		/* foodtype */
		$registered_string_ids[] = $this->wpml_register_foodtype($wppizza_options);		
		/* additives */
		$registered_string_ids[] = $this->wpml_register_additives($wppizza_options);
		/* allergens */
		$registered_string_ids[] = $this->wpml_register_allergens($wppizza_options);
		/* sizes */
...
***********************************************************************************************/
	...
	*/
	...
	/*
		allergens
	*/
	function wpml_register_allergens($set_options, $unregister = false){
		$registered_string_ids = array();

		/* unregister obsolete strings */
		if($unregister){
			global $wppizza_options;
			$current = $wppizza_options['allergens'];
			$obsolete = array_diff_key($current, $set_options['allergens']);
			if(!empty($obsolete)){
			foreach($obsolete as $obsolete_key => $arr){
				icl_unregister_string(WPPIZZA_SLUG,'allergens_'. $obsolete_key.'_name');
				icl_unregister_string(WPPIZZA_SLUG,'allergens_'. $obsolete_key.'_sort');
			}}
		}

		if(!empty($set_options['allergens'])){
		arsort($set_options['allergens']);/* reverse sort to - somewhat - correspond with order set */
		foreach($set_options['allergens'] as $k=>$arr){
			if($arr['name'] != ''){
				$registered_string_ids[] = icl_register_string(WPPIZZA_SLUG,'allergens_'. $k.'_name', $arr['name']);
			}
			if($arr['sort'] != ''){
				$registered_string_ids[] = icl_register_string(WPPIZZA_SLUG,'allergens_'. $k.'_sort', $arr['sort']);
			}
		}}
	...
	*/
	...
	/*
		foodtype
	*/
	function wpml_register_foodtype($set_options, $unregister = false){
		$registered_string_ids = array();

		/* unregister obsolete strings */
		if($unregister){
			global $wppizza_options;
			$current = $wppizza_options['foodtype'];
			$obsolete = array_diff_key($current, $set_options['foodtype']);
			if(!empty($obsolete)){
			foreach($obsolete as $obsolete_key => $arr){
				icl_unregister_string(WPPIZZA_SLUG,'foodtype_'. $obsolete_key.'_name');
				icl_unregister_string(WPPIZZA_SLUG,'foodtype_'. $obsolete_key.'_sort');
			}}
		}

		if(!empty($set_options['foodtype'])){
		arsort($set_options['foodtype']);/* reverse sort to - somewhat - correspond with order set */
		foreach($set_options['foodtype'] as $k=>$arr){
			if($arr['name'] != ''){
				$registered_string_ids[] = icl_register_string(WPPIZZA_SLUG,'foodtype_'. $k.'_name', $arr['name']);
			}
			if($arr['sort'] != ''){
				$registered_string_ids[] = icl_register_string(WPPIZZA_SLUG,'foodtype_'. $k.'_sort', $arr['sort']);
			}
		}}
	...
	*/
	...
	/*
		add string translation for selected language (wppizza->tools)
		this is not officially supported by WPML , 
		https://wpml.org/forums/topic/cant-translate-string-programmatically/
		
		but if we ever want to do this anyway we should look for 
		the 'icl_add_string_translation' or 'icl_update_string_translation' function in 
