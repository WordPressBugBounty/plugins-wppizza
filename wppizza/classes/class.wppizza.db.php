...
class WPPIZZA_DB{
	...
	**********************************************/
	...
	/**************************************************************************************************
		...
		update order
		@since 3.18.12 using $wpdb->query writing our own query string
		instead of $wpdb->update so we can use "IN('a','b','c' )" in WHERE clauses ($wpdb->query alwasy uses "=" )
	**************************************************************************************************/
	function update_order($blog_id, $order_id, $hash, $update_values, $where_payment_status = false) {
		...
		**/
		...
		if($order_id){
			$where['id'] = $order_id;
			$where_format['id'] = '%d';
			$where_clause['id'] = $wpdb->prepare("id = ".$where_format['id']."", $where['id']);
		}
		if($hash){
			$where['hash'] = $hash;
			$where_format['hash'] = '%s';
			$where_clause['hash'] = $wpdb->prepare("hash = ".$where_format['hash']."", $where['hash']);
		}
		...
		**/
		if(!empty($where_payment_status)){
			...
			$where_format['payment_status'] = '%s';
			//allow for IN clauses)
			if(is_array($where_payment_status)){
				$in_clause = array();
				foreach($where_payment_status as $status){
					$in_clause[] = $wpdb->prepare( $where_format['payment_status'], $status);
				}
				$where_clause['payment_status'] = "payment_status IN(".implode(",", $in_clause).")";
			}else{
				$where_clause['payment_status'] = $wpdb->prepare("payment_status = ".$where_format['payment_status']."", $where['payment_status']);
			}
		}
		...
		*/
		...
		foreach($update_values as $key=>$val){
			...
			$order['type'][$key] = $val['type'];
			$update_data[$key] = $wpdb->prepare("".$key." = ".$val['type']."", $val['data']);
		}

		/*
			create query 
		*/
		$update_order_query		= "UPDATE " . $order_table . " SET ";
		if(!empty($update_data)){
			$update_order_query	.= implode(', ', $update_data);
		}
		if(!empty($where_clause)){
			$update_order_query	.= " WHERE " . implode(" AND ", $where_clause);
		}
		...
		/*
			update order
		*/
		$update_order = $wpdb->query( $update_order_query );
	...
	**************************************************************************************************/
	function get_orders($args = false, $caller = false){
		...
		/****************************************************************
			allow filtering of query arguments passed (if not empty, for now)
			@since 3.16.4
		****************************************************************/
		if(!empty($args)){
			$args = apply_filters('wppizza_filter_get_orders_args', $args, $caller);
			
			// alternative filter that can be used - includes $caller in filter name
			if(is_string(trim($caller)) && !empty(trim($caller))){
			$args = apply_filters('wppizza_filter_get_orders_args_'.$caller.'', $args);
			}
		}
	...
	*
	**************************************************************************************************/
	function map_order($user_session, $recalculate_cart = false, $checkout_parameters_only = false) {
		...
		*/
		$order_session = WPPIZZA()->session->sort_and_calculate_cart(null, $recalculate_cart, __FUNCTION__);
		...
		*/
		...
		$transaction_errors 		= '';
		$display_errors 			= '';
		$validate_initiator 		= empty($user_session[''.WPPIZZA_SLUG.'_gateway_selected']) ? 'COD' : wppizza_validate_alpha_only($user_session[''.WPPIZZA_SLUG.'_gateway_selected']);/* php 5.3 */
