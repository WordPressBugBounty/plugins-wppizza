...
class WPPIZZA_ORDER_EXECUTE{
	...
	function order_redirect($order_details){
		...
		***********************************************************/
		...
		if (class_exists('WPPIZZA_GATEWAY_'.$gw_id_upper)){
		    ...
		    $gateway_class = new $class_name($gw_id_lower);
	...
	*********************************************************************************************************/
	function order_execute($order_details, $tx_id = null, $tx_details = false, $tx_errors = false, $ipn = false, $check_user_id = null, $custom_update_columns = false, $unconfirmed = null, $send_emails = true){
		...
		****************************************************************/
		...
		// since wppizza 3.18.12. setting an array('INPROGRESS', 'CONFIRMED') here instead of just looking for 'INPROGRESS' in the WHERE clause as in some circumstances the payment status *may* already be set to CONFIRMED and we do not want this to cuase an error
		$inprogress_status = ($unconfirmed === null || $unconfirmed === true ) ? array('INPROGRESS', 'CONFIRMED') : 'UNCONFIRMED' ;	
		...
		*/
		if($send_emails === true ){
			...
			/*
				...
				(Note: $email_results is actually an array of any $mail_errors returned . If there are no problems, it's an empty array ) 
			*/
	...
	*********************************************************************************************************/
	function order_failed($order_id, $transaction_details = false, $order_details = false, $transaction_id = false, $transaction_errors = false, $display_errors = false){
		...
		****************************************************************/
		...
		/** transaction errors */
		$update_db_values['transaction_errors'] = array('type'=> '%s', 'data' => (!empty($transaction_errors) ? $this->capture_tx_errors($order_id, $transaction_errors) : null) );
	...
	******************************************************************/
		...
		**************************************************************/
		if($is_critical && !empty($order_id) && !empty($tx_errors) && !empty($fail_order_on_error)){
			...
			*******************/
			...
			$track_logentry =  print_r($track_order, true) . ' ' . implode('', $print);
	...
	/******************************************
		since 3.18.11, we are also capturing the state of the order *before* updating to FAILED 
		to help us trace things back to what it was before  
	*****************************************/
	function capture_tx_errors($order_id, $tx_errors){
		global $wpdb;
		
		/* init */
		$txErrors = array();
		$txErrors['tx_errors'] = !empty($tx_errors) ? $tx_errors : '' ;
		
		/* 
			get current state of order 
		*/
		$order_state = $wpdb->get_row( "SELECT * FROM ".$wpdb->prefix . WPPIZZA_TABLE_ORDERS . " WHERE id =  ".(int)$order_id."", ARRAY_A);
		/* 
			combine transation errors with order state as it was before updating 
			Mavbe we want to add some other tracing info at some point , so let's add it as 'order_state' key 
		*/
		$txErrors['trace'] = array(
			'order_state' => !empty($order_state) ? $order_state : 'Order ['.$order_id.'] - Does not exist !!',
		);			

	return maybe_serialize($txErrors);
	}
	...
	function get_email_templates($order_formatted){
		...
		*/
		...
		foreach($enabled_formfields as $cdataKey=>$cDataVal){
			if(!empty($cDataVal['add_to_subject_line'])){
				...
				$set_user_value = empty($order_formatted['sections']['customer'][$cdataKey]['value']) ? '' : trim($order_formatted['sections']['customer'][$cdataKey]['value']);
		...
		$customer_data_add_to_subject = empty($customer_data_add_to_subject) ? '' : trim(implode(' ',$customer_data_add_to_subject));
		...
		***************/
		...
		if(!empty($order_formatted['sections']['customer']['cemail']['value'])){
			...
			/* set reply to as well to be customer */
	...
	***************************************************************/
	function get_print_templates($tpl_ids, $order_id, $order_formatted){
		...
		*/
		...
		$template_data = array();//re-init as array
