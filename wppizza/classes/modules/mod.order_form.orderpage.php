		...
		if($help){
			$settings['help'][$this->section_key][] = array(
				...
				'description'=>array(
					...
					__('For "selects", the "placeholder" field can be used to replace the default "please select" .This will also be used as error message if a selection is required.', 'wppizza-admin'),
					__('"Checkboxes" and "radio" inputs will furthermore use the "placeholder" as title attribute for the surrounding html element.', 'wppizza-admin'),
					__('If you want to omit a field from the order form when it is not required, enable "Omit if optional".', 'wppizza-admin'),
					...
					__('Additionally, if you want to offer customers to be able to register a new account on the order page - provided they are not logged in already - the email field must be set to be "enabled" and "required". Additionally "Anyone can register" in "Settings->General" has to be enabled too.', 'wppizza-admin')
				)
			);
		}
			...
			foreach($input_fields as $k=>$v){
				...
				*/
				...
				if($v['key']=='ctips'){
					...
					$attr['omit_if_optional'] = false;//for the time being lets not use it for tips
				}
				...
				echo"<tr class='wppizza_".$v['key']."'>";
					...
					echo"<td>";
						...
						/* omit if optional */
						if(!empty($attr['omit_if_optional'])){echo"<label><input name='".WPPIZZA_SLUG."[".$options_key."][".$k."][omit_if_optional]' type='checkbox' ". checked(!empty($v['omit_if_optional']),true,false)." value='1' />".__('Omit if optional', 'wppizza-admin')."</label>";}else{echo"";}
					echo"</td>";
			...
			*****************************************/
			...
			foreach($input[$this->settings_page] as $key=>$b){
				...
				*/
				...
				$options[$this->settings_page][$key]['omit_if_optional'] 	= !empty($input[$this->settings_page][$key]['omit_if_optional']) ? true : false;
...
***************************************************************/
	function get_validation_rules(){
		...
		/*
			// customise rules
			// see jqueryvalidation.org what each rule/method does
